function minimal_3cycles(V::Common.Points, EV::Common.ChainOp, FE::Common.ChainOp)
    ...
    function face_angle(e::Int, f::Int)
        if !isassigned(triangulated_faces, f)
            ...
            #vs = V[vs_idxs, :]
            fv, edges = Arrangement.vcycle(EV, FE, f)
            ...
            trias = Arrangement.triangulate2d(v, edges)
        ...
        v1 = Common.normalize(V[edge_vs[2], :] - V[edge_vs[1], :])
        ...
        v = vs[3, :] - vs[1, :]
        angle = Common.atan(v[2], v[3])
        return angle
    end

    #EF = FE'
    EF = convert(Common.ChainOp, Common.transpose(FE))

    FC = Arrangement.minimal_cycles(face_angle)(V, EF)  # , EV)

    #FC'
    return -convert(Common.ChainOp, Common.transpose(FC))
end
